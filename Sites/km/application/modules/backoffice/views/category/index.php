...
<table>
	<thead>
        <tr>
            ...
            <th scope="col" class="rounded" width="350">หมวดหมู่บทความ</th>
	...
	?>      
    							<tbody>
                                    <tr>
                                        <!--<td>< ?php echo form_checkbox(array('name'=>'chkbox[]','id'=>'chkbox[]','value'=>$row->CAT_id)) ?></td>-->
                                        <td style="text-align:center">
                                        			<input name="order[]" value="<?php echo $row->CAT_order; ?>" class="text" size="3" maxlength="3" />
                                        			<input name="keyed[]" type="hidden" value="<?php echo $row->CAT_id; ?>"/>
                                        </td>
                                        <td><?php echo $row->CAT_topic; ?></td>
                                        <td style="text-align:center">
                                        <?php 
                                            $pic = ( $row->CAT_activated == '1' ?  '<img src="'.site_url('asset/backoffice/images/icons/accepted_24.png').'" />' :  
                                            '<img src="'.site_url('asset/backoffice/images/icons/cancel_24.png').'" />'); 
                                            echo anchor("backoffice/category/activated/".$row->CAT_id."/".$row->CAT_activated,$pic,array('onclick' => 'return active_data()')); 
                                        ?></td>
                                        <td style="text-align:center"><?php echo th_date($row->CAT_update->format('Y-m-d H:i:s'));  ?></td>
                                        <td style="text-align:center"><?php echo $row->CAT_userupdate; ?></td>
                                        <td style="text-align:center">
			...
			$(function(){
					...
					$('span.delete').click(function(){
							if (confirm("คุณต้องการลบข้อมูลหรือไม่ ?")) {
								$('#grid').attr('action','<?php echo site_url('backoffice/category/delete'); ?>');
								$('#grid').submit();
							}
					});
					$('span.order').click(function(){
							if (confirm("คุณต้องการเรียงลำดับข้อมูลหรือไม่ ?")) {
								$('#grid').attr('action','<?php echo site_url('backoffice/category/order'); ?>');
								$('#grid').submit();
							}
					});
			});
